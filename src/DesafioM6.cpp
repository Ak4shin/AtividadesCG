...
struct Objeto3D
{
    ...
    int pontoAtual = 0;
    float tTrajetoria = 0.0f;
    float velocidadeTrajetoria = 1.0f;    
...
void atualizarTrajetoria(Objeto3D& obj, float dt)
{
    if (!obj.trajetoriaAtiva || obj.pontosTrajetoria.size() < 2)
        return;

    int proximoPonto = (obj.pontoAtual + 1) % obj.pontosTrajetoria.size();

    glm::vec3 origem = obj.pontosTrajetoria[obj.pontoAtual];
    glm::vec3 destino = obj.pontosTrajetoria[proximoPonto];

    float distancia = glm::distance(origem, destino);

    if (distancia <= 0.001f)
    {
        obj.pontoAtual = proximoPonto;
        obj.tTrajetoria = 0.0f;
        obj.posicao = destino;
        return;
    }

    obj.tTrajetoria += (obj.velocidadeTrajetoria / distancia) * dt;

    if (obj.tTrajetoria >= 1.0f)
    {
        obj.posicao = destino;
        obj.pontoAtual = proximoPonto;
        obj.tTrajetoria = 0.0f;
    ...
    obj.posicao = glm::mix(origem, destino, obj.tTrajetoria);
}
...
void key_callback(GLFWwindow* window, int key, int scancode, int action, int mode)
{
    ...
    {
        obj.pontosTrajetoria.push_back(obj.posicao);

        cout << "Ponto adicionado ao objeto " << obj.nome << ": ";
        cout << obj.posicao.x << ", " << obj.posicao.y << ", " << obj.posicao.z << endl;
        cout << "Total de pontos: " << obj.pontosTrajetoria.size() << endl;
    }

    if (key == GLFW_KEY_SPACE && action == GLFW_PRESS)
    {
        if (obj.pontosTrajetoria.size() >= 2)
        {
            obj.trajetoriaAtiva = !obj.trajetoriaAtiva;
            obj.pontoAtual = 0;
            obj.tTrajetoria = 0.0f;

            cout << "Trajetoria do objeto " << obj.nome << ": ";
            cout << (obj.trajetoriaAtiva ? "ativa" : "pausada") << endl;
        }
        else
        {
            cout << "Adicione pelo menos 2 pontos com P para iniciar a trajetoria." << endl;
        }
    }

    if (key == GLFW_KEY_C && action == GLFW_PRESS)
    {
        obj.pontosTrajetoria.clear();
        obj.trajetoriaAtiva = false;
        obj.pontoAtual = 0;
        obj.tTrajetoria = 0.0f;

        cout << "Pontos da trajetoria apagados para " << obj.nome << endl;
    }
}
...
void imprimirControles()
{
    ...
    cout << "Setas: mover objeto selecionado em X/Y" << endl;
    cout << "Page Up / Page Down: mover objeto em Z" << endl;
    cout << "P: adicionar ponto na trajetoria do objeto selecionado" << endl;
    cout << "SPACE: iniciar/pausar trajetoria do objeto selecionado" << endl;
    cout << "C: limpar pontos da trajetoria do objeto selecionado" << endl;
    cout << "ESC: sair" << endl;
}