...
int main()
{
    ...
    GLFWwindow* window = glfwCreateWindow(WIDTH, HEIGHT, "Atividade Vivencial 2 - Iluminacao de 3 Pontos", nullptr, nullptr);
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        glm::vec3 posPrincipal = objetos[0].posicao;
        float escalaPrincipal = max(objetos[0].escala.x, max(objetos[0].escala.y, objetos[0].escala.z));

        glm::vec3 lightPositions[3];

        // Luz principal: mais forte, na frente e em um dos lados
        lightPositions[0] = posPrincipal + glm::vec3(-3.0f, 2.5f, 3.0f) * escalaPrincipal;

        // Luz de preenchimento: mais fraca, no lado oposto
        lightPositions[1] = posPrincipal + glm::vec3(3.0f, 1.5f, 2.5f) * escalaPrincipal;

        // Luz de fundo: atrás e acima do objeto principal
        lightPositions[2] = posPrincipal + glm::vec3(0.0f, 3.0f, -3.0f) * escalaPrincipal;

        glm::vec3 lightIntensities[3];
        lightIntensities[0] = glm::vec3(1.00f, 0.95f, 0.85f); // key light
        lightIntensities[1] = glm::vec3(0.35f, 0.40f, 0.50f); // fill light
        lightIntensities[2] = glm::vec3(0.65f, 0.75f, 1.00f); // back light
